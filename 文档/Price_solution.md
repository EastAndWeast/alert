...
| **最大报价停滞时间** | Max Stale Time (Sec) | Number | 是 | 连续多少秒未收到新 Tick 报价则告警（如 `10`秒）。防范断线与网关宕机。 |
...
| **Alert Type** | 具体的异常标签 | `<span class="badge badge-secondary">Pricing</span>` |
| **Current Value/Summary** | 触发时的实际现状与概要 | `"15s \| 停价"` (在触发值与Details列中展示) |
| **Threshold** | 用户设定的触发红线 | `"> 10s"` |
