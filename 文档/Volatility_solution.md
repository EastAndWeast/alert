...
| **时间窗口** | Time Window (Min) | Select/Number | 是 | 监测的聚合周期，最常见为 `1 Min`, `5 Min`, `15 Min` |
| **最大波幅阈值** | Max Fluctuation (Points) | Number | 是 | 在时间窗口内，最高价与最低价之差大于此点数（如 `500`点）即告警。 |
| **通知对象** | Notification Group | Select | 否 | 指定接收告警的用户组（通常由于是市场行为引发，建议默认发给业务风控盯盘组）。 |
...
| **Time Window** | 触发的统计时长 | `<span class="badge">1 Min Window</span>` |
| **Actual Fluctuation** | 窗口内的实际波动数值 | `"Changed by 800 pts"` |
| **Threshold** | 用户设定的高危线 | `"> 500 pts"` |
