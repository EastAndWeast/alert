...
| **pricing** | `45s` | `trigger_value: 45, details: {alert_subtype: 'PRICING'}` | ✅ 需要 |
| **volatility** | `85点` | `trigger_value: 85, details: {alert_subtype: 'VOLATILITY'}` | ✅ 需要 |
...
```javascript
{
  trigger_value: 85,           // 波动数值
  details: { change_points: 85, time_window: 'M1' }
}
```
...
```javascript
{
  ...
  details: { net_position: 62, direction: 'LONG' }
}
```
...
formatTriggerValue(alert) {
    ...
    // 7. Volatility
    if (t === 'volatility') {
        var unit = d.change_points ? I18n.t('unit_points') : '%';
        var val = d.change_points || d.change_percentage || v;
        return val + unit + ' | ' + I18n.t('volatility');
    }
